...
void Engine::TextureManager::LoadTexture(const std::string& filePath) {
	...
	if (DirectX::IsCompressed(image.GetMetadata().format) || isHdr) { // 圧縮フォーマットかどうかを調べる
	...
	if (isHdr) {
		...
		// キューブマップとして設定
		textureData.metadata.mipLevels = 1;  // 通常1レベルのミップマップ
		textureData.metadata.dimension = DirectX::TEX_DIMENSION_TEXTURE2D; // 2D テクスチャとして扱う
