...
#pragma region CrowdAgent
		...
		const std::vector<CrowdAgent>* allAgents)
	{
		...
		position_ = owner->GetObjectComponent()->GetWorldPosition();
		...
		case AgentState::PreparationAttack:
		{
			if (attackDelayTimer_ <= 0.0f) {
				...
				owner->GetCharacterStateMachine()->ChangeState(CharacterMainState::Attack);
		...
		case AgentState::Attack:
		{
			...
			if (owner->GetCharacterStateMachine()->GetCurrentMainState()
...
#pragma region CrowdManager
	...
	void CrowdManager::UpdateAgentsToInstancing()
	{
		for (auto& agent : agents) {
			if (agent.owner && !agent.isDed) {
				if (agent.owner->GetHP() <= 0) {
					agent.isDed = true;
				}
			}
		}
