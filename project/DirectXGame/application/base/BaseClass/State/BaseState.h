...
private:
    AttackSubState subState_ = AttackSubState::Ready;
    float timer_ = 0.0f;
...
public:
    ...
    void Enter() override {
        faintTimer_ = 0.0f;
        // アニメーション開始
        // character_->PlayAnimation("Faint");
        // 操作不能に
        // character_->SetControllable(false);
    }

    void Update() override {
        //faintTimer_ += character_->GetDeltaTime();
        //if (faintTimer_ >= faintDuration_) {
        //    // 復帰
        //    // character_->SetControllable(true);
...
private:
    ...
    float faintDuration_ = 3.0f; // 3秒で回復など
};
