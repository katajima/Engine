...
void PlayerRangeBombingBullet::Initialize(Entity3DManager* entity3DManager, Entity2DManager* entity2DManager, GlobalVariables* globalVariables, Vector3 position, Camera* camera)
{
	...
	//effectComponent_->AddTrailEffect("trail", "resources/Texture/Image.png", 0.5f, object_->GetWorldTransform(), { 1.0f,1.0f,1.0f,1.0f }, { 0,-10.5f,0 }, { 0,10.5f,0 });
...
void PlayerRangeBombingBullet::Update()
{
	...
	emitterPoint->GetWorldTransform().translate_ = strSmoke;
	emitterPoint->GetCommonData()->prevTranslate = endSmoke;
