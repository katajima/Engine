...
void CollisionManager::CheckCollisionPair(Collider* colliderA, Collider* colliderB) 
{
	...
	// カプセル同士 
	else if (colliderA->GetColliderType() == static_cast<uint32_t>(ColliderType::Capsule) &&
		colliderB->GetColliderType() == static_cast<uint32_t>(ColliderType::Capsule)) {
		CollFanction::CapsuleAndCapsule(colliderA, colliderB);
	}
	// AABB同士
	else if (colliderA->GetColliderType() == static_cast<uint32_t>(ColliderType::AABB) &&
		colliderB->GetColliderType() == static_cast<uint32_t>(ColliderType::AABB)) {
		CollFanction::AABBAndAABB(colliderA, colliderB);
	}
